...
void CubemapFramebufferSample::OnInit(){
    ...
    mat1 = CreateRef<Material>(AssetManager::Get().LoadAsset<Shader>("Engine/Shaders/Unlit.glsl"));
    mat1->SetTexture("mainTex", AssetManager::Get().LoadAsset<Texture2D>("Sandbox/Textures/Rock.jpg"));
    mat1->SetVector4("color", {1,1,1,1});

    mat2 = CreateRef<Material>(AssetManager::Get().LoadAsset<Shader>("Sandbox/Shaders/CubemapFramebufferSample.glsl"));

    model = AssetManager::Get().LoadAsset<Model>("Sandbox/Models/cube.gltf");
    model2 = AssetManager::Get().LoadAsset<Model>("Engine/Models/Sphere.obj");
    ...
    skyMat = CreateRef<Material>(AssetManager::Get().LoadAsset<Shader>("Engine/Shaders/SkyboxCubemap.glsl"));
    skyMat->SetCubemap("mainTex", AssetManager::Get().LoadAsset<Cubemap>("DefaultSkyboxCubemap"));

    fullscreenQuadMesh = Mesh::FullScreenQuad();
    screenPassMat = CreateRef<Material>(AssetManager::Get().LoadAsset<Shader>("Engine/Shaders/GamaCorrectionPP.glsl"));
}
