...
void LightSample::OnInit(){
    ...
    FramebufferRenderPass::RegisterRenderPass("TestPass", renderPassInfo);

    camMove.transform = &camTransform;

    modelTransform.Position(Vector3Zero);
    ...
    model->materials[0]->SetVector3("light_position", lightTransform.Position());
    model->materials[0]->SetVector3("viewPos", camTransform.Position());
    ...
    framebuffer = ResourceManager::Get().Create<Framebuffer>("TestPass", Application::ScreenWidth(), Application::ScreenHeight());
...
void LightSample::OnUpdate(float deltaTime){
    camMove.OnUpdate();
    modelTransform.EulerAngles(Vector3(0, Platform::GetTime() * 40, 0));
}   
...
void LightSample::OnGUI(){
    ...
    lightTransform.Position(Vector3(translation[0], translation[1], translation[2]));
