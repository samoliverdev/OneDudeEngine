...
void ChunkBuilderLayer::BuildData(IVector3 coord, ChunkDataHolder& data){
    for(int z = 0; z < data.chunkData->GetSize(); z++){
        for(int x = 0; x < data.chunkData->GetSize(); x++){
            ...
            float noiseValue = noise.GetNoise(x*noiseScale, _z*noiseScale); //fnlGetNoise2D(&noise, _x*noiseScale, _z*noiseScale);
            ...
            float treeNoiseValue = noise.GetNoise((_x+treeNoiseOffset)*treeNoiseScale, (_z+treeNoiseOffset)*treeNoiseScale);
