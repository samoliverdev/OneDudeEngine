...
void DrawAnimationCurveEditor(AnimationCurve& curve, ImVec2 size) {
    ...
    // Time and value ranges for clamping
    ...
    // Time and value ranges for drawing (with 0.2 border)
    float draw_min_time = clamp_min_time - 0.2f;
    float draw_max_time = clamp_max_time + 0.2f;
    ...
    if (is_hovered && ImGui::IsMouseClicked(ImGuiMouseButton_Right)) {
        ...
        // Clamp to original ranges when adding keyframes
