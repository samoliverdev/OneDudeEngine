...
void PhysicsSystem::Update(){
    ...
    for(auto e: heightView){
        ...
        TransformComponent& transform = heightView.get<TransformComponent>(e);

        if(rb.shape == nullptr){;
            rb.shape = new btHeightfieldTerrainShape(
                rb.width, rb.length, &rb.heights[0], 1.0, rb.minHeight, rb.maxHeight, 1, PHY_FLOAT, false
            );
            rb.shape->setLocalScaling(ToBullet(Vector3One));

            rb.body = new btCollisionObject();
            rb.body->setCollisionShape(rb.shape);

            //btTransform groundTransform;
            //groundTransform.setIdentity();
            //groundTransform.setOrigin(ToBullet(transform.Position() + rb.offset));
            //rb.body->setWorldTransform(groundTransform);

            physicsWorld->world->addCollisionObject(rb.body);
            ...
            btRigidBody::btRigidBodyConstructionInfo rbInfo(mass, motionState, rb.heightShape, localInertia);
            
            rb.terrainRigidBody = new btRigidBody(rbInfo);
            
            physicsWorld->world->addRigidBody(rb.terrainRigidBody);*/
        } else {
            rb.shape->setLocalScaling(ToBullet(transform.Scale()));
            btTransform physicsTransform = ToBullet(transform);
            rb.body->setWorldTransform(physicsTransform);
        }
    }
