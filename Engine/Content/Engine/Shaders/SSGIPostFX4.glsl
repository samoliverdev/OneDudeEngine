...
float Bayer4x4Centered(vec2 pixel){
    return Bayer4x4(pixel) - 0.5;
}
...
float Bayer8x8(vec2 pixel)
{
    ...
    return (bayer[y * 8 + x] + 0.5) / 64.0;
}

// Returns [-0.5, 0.5]
float Bayer8x8Centered(vec2 pixel)
{
    return Bayer8x8(pixel) - 0.5;
}
...
vec3 HorizonSampling(
    ...
    for(int i = 0; i < _StepCount; i++){
        ...
        if(hits > 0){
            ...
            float maxLuminance = 0.1; //0.25; // SAME as three.js
