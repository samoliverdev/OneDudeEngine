...
Texture2D(0, 14, giSurface, giSurfaceSampler);
    ...
    float SpatialWeight(vec2 offset){
        ...
        return exp(-d2 * spatialSigma);
    }

    float SpatialWeight(vec2 offset, vec2 fracPos){
    ...
    void main(){
        ...
        // Center surface
        float centerDepth = Linear01Depth(texture(gDepth, centerUV).r);
        vec3 centerNormal = GetNormal(centerUV);
        ...
        int searchSize = int(floor(scaleX + 0.5));
        searchSize = max(searchSize, 1);

        // Accumulation
        float bestScore = 1e30;
        float bestDepth = centerDepth;
        vec3 bestNormal = centerNormal;
        ...
        for(int y = 0; y < 4; y++){
            for(int x = 0; x < 4; x++){
                ...
                // 0 1 2 3
                // ...
                vec2 sampleOffset = vec2(float(x), float(y)) - (float(searchSize) - 1.0) * 0.5;

                // Full-res sample UV
                vec2 sampleUV = centerUV + sampleOffset * screenTexelSize;
                sampleUV = clamp(sampleUV, screenTexelSize * 0.5, vec2(1.0) - screenTexelSize * 0.5);

                // Sample geometry
                float sampleDepth = Linear01Depth(texture(gDepth, sampleUV).r);
                vec3 sampleNormal = GetNormal(sampleUV);

                // Depth similarity
                float depthDifference = abs(sampleDepth - centerDepth);
                float relativeDepth = depthDifference / max(centerDepth, 1e-4);
                float depthScore = relativeDepth * depthSigma;

                // Normal similarity
                float normalDot = max(dot(centerNormal, sampleNormal), 0.0);
                float normalDifference = 1.0 - normalDot;
                float normalScore = normalDifference * normalSigma;

                // Spatial preference
                float spatialDistance = dot(sampleOffset, sampleOffset);
                float spatialScore = spatialDistance * spatialSigma;
        ...
        // RGB = representative normal
        // A   = representative linear depth
        fragColor = vec4(bestNormal, bestDepth);
    }
    #endif
    ...
    vec4 BilateralUpsample(vec2 uv){
        ...
        vec2 lowPosition = uv * giSize - 0.5;
        vec2 basePosition = floor(lowPosition);
        vec2 fracPosition = fract(lowPosition);
        ...
        for(int y = -RADIUS; y <= RADIUS; y++){
            for(int x = -RADIUS; x <= RADIUS; x++){
                ...
                vec2 samplePosition = basePosition + offset + vec2(0.5);

                vec2 sampleUV = samplePosition / giSize;
                sampleUV = clamp(sampleUV, giTexelSize * 0.5, vec2(1.0) - giTexelSize * 0.5);

                // GI/AO
                vec4 giSample = texture(giLow,sampleUV);

                // Surface-aware low-res geometry
                vec4 surface = texture(giSurface, sampleUV);

                vec3 sampleNormal = normalize(surface.rgb);
                float sampleDepth = surface.a;
                ...
                // reconstruction grid.
                vec2 spatialOffset = offset - fracPosition;
                float spatialWeight = SpatialWeight(spatialOffset);
    ...
    void main(){
        fragColor = BilateralUpsample(texCoord);
    }
    #endif
    ...
    vec4 BilateralUpsample(vec2 uv){
        ...
        for(int y = -RADIUS; y <= RADIUS; y++){
            for(int x = -RADIUS; x <= RADIUS; x++){
                ...
                // Spatial
                float spatialWeight = SpatialWeight(offset, fracPosition);
                // Depth
                float depthWeight = DepthWeight(centerDepth, sampleDepth);
                // Normal
                float normalWeight = NormalWeight(centerNormal, sampleNormal);
